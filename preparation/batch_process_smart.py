...
class MaskConfig:
    ...
    blur_kernel: tuple = (11, 11)      # smoothness kernel
    canny_low: int = 5                
    canny_high: int = 60               
    
    # Erosion, Dilatation for cany
    close_kernel: tuple = (11, 11)   
    close_iterations: int = 10         
    dilate_iterations: int = 1         
    
    # extra parameters
    border_thickness: int = 10         # black border to detach mask from image edges
    smooth_blur: tuple = (21, 21)      # smooth mask
    open_kernel: tuple = (5, 5)        # remove noise
...
def build_rope_mask(img_bgr, keep_main_component=True):
    ...
    if keep_main_component:
        mask_main, largest_area = keep_largest_component(mask_clean)
    else:
        mask_main = mask_clean.copy()

    meta = {
        'best_channel': 'Canny_Edges_Detached',
        'best_score': 1.0,
        'largest_component_area': largest_area,
        'mask_raw': edges,
        'mask_closed': edges_dilated,
        'mask_clean': mask_clean,
        'img_flat': img_bgr  
    }
    return mask_main, meta
...
def find_knot_roi(mask, min_side=220, density_kernel=101, density_thresh_factor=0.8, debug_prefix=None, orig_img=None):
    if debug_prefix is not None:
        debug_prefix = Path(debug_prefix)
    ...
    if max_den == 0.0:
        raise RuntimeError("Empty mask: no white pixels found.")
...
def process_image(img_path):
    ...
    try:
        ...
        mask, meta = build_rope_mask(img)
        roi, hot, density = find_knot_roi(mask, min_side=224, density_thresh_factor=0.6, orig_img=img)
        ...
        if knot_crop.size == 0 or knot_mask_rough.size == 0:
            raise ValueError("Ungültige ROI (leeres Crop)")
...
def main():
    input_dir = Path("C:/Users/haran/Desktop/8knot_images")
    output_dir = Path("dataset_processed")
    classes = ["correct", "wrong"]

    for cls in classes:
        in_path = input_dir / cls
        out_path = output_dir / cls

        if not in_path.exists(): continue
        out_path.mkdir(parents=True, exist_ok=True)

        image_files = list(in_path.glob("*.jpg")) + list(in_path.glob("*.jpeg")) + list(in_path.glob("*.png"))
        print(f"\nPrüfe Klasse '{cls}' ({len(image_files)} Bilder in RAW)...")

        processed_count = 0
        skipped_count = 0

        for img_file in image_files:
            expected_output_name = f"{img_file.stem}_cnn.png"
            ...
            cnn_img = process_image(img_file)

            if cnn_img is not None:
                cv2.imwrite(str(expected_output_path), cnn_img)
                processed_count += 1

        print(f"Fertig für '{cls}'. Neu verarbeitet: {processed_count}, Übersprungen: {skipped_count}.")
